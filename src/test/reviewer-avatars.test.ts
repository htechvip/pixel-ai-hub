...
describe('Reviewer Avatar URLs', () => {
  ...
  it('should have accessible avatar URLs from actual Course component implementation', async () => {
    // Test the actual avatar URLs used in Course.tsx component
    const actualAvatarUrls = Object.values(actualReviewerAvatars);
    console.log(`Testing ${actualAvatarUrls.length} URLs from Course.tsx component...`);
    
    const results = await checkMultipleImages(actualAvatarUrls, 3);
    
    const brokenImages = results.filter(result => !result.ok || !result.isImage);
    
    console.log(`Found ${brokenImages.length} broken images out of ${results.length} total`);
    if (brokenImages.length > 0) {
      console.log('Broken images:');
      brokenImages.forEach(img => console.log(`  - ${img.url}: ${img.error || 'Not accessible'}`));
    }
    
    if (brokenImages.length > 0) {
      const brokenUrls = brokenImages.map(img => `${img.url} (${img.error || 'Not accessible'})`);
      expect.fail(`Found ${brokenImages.length} broken avatar images in Course.tsx:\n${brokenUrls.join('\n')}`);
    }
